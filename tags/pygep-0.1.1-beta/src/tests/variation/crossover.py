...
class CrossoverTest(unittest.TestCase):
    ...
    def _verify(self, p1, p2, c1, c2):
        for i in xrange(len(c1)):
            self.assertTrue(c1.chromosome[i] in [p1[i], p2[i]])
            self.assertTrue(c2.chromosome[i] in [p1[i], p2[i]])
            self.assertEqual(p1[i]==p2[i], c1.chromosome[i]==c2.chromosome[i])


    def testCrossoverOnePoint(self):
        c1, c2 = self.chromosome1.crossover_one_point(self.chromosome2)
        self._verify(self.chromosome1.chromosome, self.chromosome2.chromosome, c1, c2)
